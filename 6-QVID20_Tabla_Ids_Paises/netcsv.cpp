...
QList<QStringList> NetCSV::filtrar_datos_existentes()
{
    ...
    //QList<int> posiciones_regiones = obtener_posiciones_regiones(tabla);
...
int NetCSV::existe_dato(QList<QStringList> tabla, QStringList dato, int posicion_inicial, int posicion_final)
{
...
QList<int> NetCSV::obtener_posiciones_paises(QList<QStringList> tabla)
{
    ...
    int id_actual = 0, i=0;
    for(; i<tabla.size();i++){
        if(tabla[i][1].toInt()!=id_actual){
            id_actual = tabla[i][1].toInt();
...
QList<int> NetCSV::obtener_posiciones_regiones(QList<QStringList> tabla)
{
    QList<int> posiciones_regiones;

    posiciones_regiones.append(0);
    int id_actual = 0;
    for(int i=0; i<tabla.size();i++){
        if(tabla[i][2].toInt()!=0 && tabla[i][2].toInt()!=id_actual){
            id_actual = tabla[i][2].toInt();
            if(posiciones_regiones.size()==id_actual-1)
                posiciones_regiones.append(0);
            posiciones_regiones.append(i);
        }
    }
    posiciones_regiones[0]=id_actual;

    return posiciones_regiones;
}
