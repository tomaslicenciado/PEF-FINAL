...
int NetCSV::existe_dato(QList<QStringList> tabla, QStringList dato)
{
    ...
    int id_pais = dato[1].toInt();
    int tabla_id_pais = 0;
    for (; bajo< alto; bajo++, media=((alto - bajo)/2)+bajo){
        tabla_id_pais = tabla[media][1].toInt();
        if (id_pais < tabla_id_pais)
            alto = media;
        else if(id_pais > tabla_id_pais)
            bajo = media +1;
        else
            break;
    }
    if ((dato[2].compare("")==0 && comp_string_fecha(dato[0],tabla[media][0])<0)||(dato[2].compare("")!=0 && dato[2].toInt()<tabla[media][2].toInt())){
        for (int i=media-1;tabla[i][1].toInt()==id_pais;i--){
            if(tabla[i].contains(dato[0]) && tabla[i][2].compare(dato[2])==0){
                if(tabla[i][3].compare(dato[3])==0)
                    return 1;
                else
                    return 2;
            }
    ...
    else if ((dato[2].compare("")==0 && comp_string_fecha(dato[0],tabla[media][0])>0)||(dato[2].compare("")!=0 && dato[2].toInt()>tabla[media][2].toInt())){
        for (int i=media+1;tabla[i][1].toInt()==id_pais;i++){
            if(tabla[i].contains(dato[0]) && tabla[i][2].compare(dato[2])==0){
                if(tabla[i][3].compare(dato[3])==0)
                    return 1;
        ...
        if(tabla[media].contains(dato[0]) && tabla[media][2].compare(dato[2])==0){
